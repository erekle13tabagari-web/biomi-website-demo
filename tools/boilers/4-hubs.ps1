...
$BRANDS = @(
  ...
  @{ brand='Riello';   slug='riello-burners'; logo='riello.svg'; cat='burners' }
)
function CatOf($x) { if ($x.cat) { return [string]$x.cat }; return 'boilers' }
# The output a card is banded by. A name with no number in it -- "RTQ 3S", a
# burner -- has a range from its family instead, and the top of it counts.
function Kw($m){
  if ($m.kw) { return [int](([string]$m.kw -split '-')[-1]) }
  $x=[regex]::Match([string]$m.name,'\b(\d{2,3})\b'); if($x.Success){return [int]$x.Groups[1].Value}; return 0
}
...
foreach ($b in $BRANDS) {
  foreach ($lang in 'ka','en') {
    ...
    $cards = ''
    foreach ($f in ($fams | Where-Object { $_.brand -eq $b.brand -and (CatOf $_) -eq $b.cat })) {
      $g = @($mods | Where-Object { $_.slug -eq $f.slug })
      if (-not $g.Count) { continue }
      $name = if ($lang -eq 'ka') { $f.nameKa } else { $f.nameEn }
      $max = ($g | ForEach-Object { Kw $_ } | Measure-Object -Maximum).Maximum
      $band = if ($max -le 35) { 'k1' } elseif ($max -lt 100) { 'k2' } else { 'k3' }
      # origin travels on data-type so the category listing can filter by it;
      # for a burner the axis that matters is the fuel, so that goes there
      $ctry = ($g | Where-Object { $_.country } | Select-Object -First 1).country
      $origin = if ($b.cat -eq 'burners') { [string]$f.fuel }
                elseif ($ctry -eq 'თურქეთი') { 'tr' } elseif ($ctry) { 'it' } else { '' }
      ...
      $kwTxt = if ($g.Count -eq 1 -and $g[0].kw) { "$($g[0].kw) kW" }
               elseif (-not $kws.Count) { '' }
    ...
    if ($b.cat -eq 'burners') {
      $grpName = $t.fFuel
      foreach ($k in 'gas','oil') {
        if ($cards -notmatch ('data-type="' + $k + '"')) { continue }
        $kwOpts += "            <label><input type=`"checkbox`" name=`"type`" value=`"$k`">$($t.$k)</label>`r`n"
      }
    } else {
      $grpName = $t.fKw
...
"@
    ...
    # BOM, like every other page; tools/add-card-brands.ps1 stamps the logos
    [IO.File]::WriteAllText((Join-Path $repo ('products\' + $b.slug + $sfx)), ($h + $body + $tl), (New-Object Text.UTF8Encoding($true)))
