...
$gals = Get-Content (Join-Path $sp 'wh-galleries.json') -Raw -Encoding UTF8 | ConvertFrom-Json

$L = @{
  ka = @{ file='.html'; tpl='vortice-lineo.html'
    home='მთავარი'; products='პროდუქტი'; cat='ბოილერი'
    ...
    thKw='მოცულობა'; thCountry='წარმოშობა'; thRange='ზომების რიგი'
  ...
  en = @{ file='-en.html'; tpl='vortice-lineo-en.html'
    ...
    thKw='Capacity'; thCountry='Origin'; thRange='Size range'
...
function Kw($name){
  $m=[regex]::Match($name,'(\d{2,4})\s*LT'); if($m.Success){return $m.Groups[1].Value}
  $m=[regex]::Match($name,'^IDRA BV\s+(\d+)'); if($m.Success){return $m.Groups[1].Value}
  if($name -match '7200[.]200NV'){ return '200' }
  return ''
}
# strip the Georgian boilerplate and the brand off the end for a chip label
function Chip($name){
  $c = $name -replace 'ბოილერი.*$','' -replace 'ბაკი.*$','' -replace 'ბუფერული.*$',''
  $c = ($c -replace '\b(BERETTA|RIELLO|OMEGA)\b','').Trim()
  if (-not $c) {
    $m = [regex]::Match($name,'[\d][\d.]*[A-Z]+')
    if ($m.Success) { $c = $m.Value }
  }
  ($c -replace '\s+',' ').Trim(' ', '-')
}
